...
class _HeatMapperStats(object):
    """Draw a heatmap plot of a matrix with nice labels and colormaps."""

    def __init__(self, data, vmin, vmax, cmap, center, robust, annot, fmt,
                 annot_kws, cellsize, cellsize_vmax,
                 cbar, cbar_kws,
                 xticklabels=True, yticklabels=True, mask=None, ax_kws=None, rect_kws=None, fdr=False):
    ...
    def plot(self, ax, cax):
        ...
        for x, y, m, val, s, annot_val in zip(xpos.flat, ypos.flat, mask.flat, data.flat, cellsize.flat, annot_data.flat):
            if not m:
                ...
                if self.annot:
                    if self.fdr:
                        annotation = f"{annot_val}"
                    else:
                        annotation = ("{:" + self.fmt + "}").format(annot_val)
        ...
        if self.cbar:
            ...
            self.cbar = cb = ax.figure.colorbar(scalar_mappable, cax, ax, **self.cbar_kws)
...
def heatmap_stats(data, vmin=None, vmax=None, cmap=None, center=None, robust=False,
                  ...
                  mask=None, ax=None, ax_kws=None, rect_kws=None, fdr=False):

    # Initialize the plotter object
    plotter = _HeatMapperStats(data, vmin, vmax, cmap, center, robust,
                               annot, fmt, annot_kws,
                               cellsize, cellsize_vmax,
                               cbar, cbar_kws, xticklabels,
                               yticklabels, mask, ax_kws, rect_kws, fdr=fdr)
